...
create index tranches_en_cours_modeles_images___fk_image
	on tranches_en_cours_modeles_images (ID_Image)
;

create index tranches_en_cours_modeles_images___modele
	on tranches_en_cours_modeles_images (ID_Modele)
;
...
create view tranches_en_cours_modeles_vue as select
    `edgecreator`.`tranches_en_cours_modeles`.`username`      AS `username`,
  ...
  from (`edgecreator`.`tranches_en_cours_modeles`
    join `edgecreator`.`tranches_en_cours_valeurs`
      on ((`edgecreator`.`tranches_en_cours_modeles`.`ID` = `edgecreator`.`tranches_en_cours_valeurs`.`ID_Modele`))) order by `edgecreator`.`tranches_en_cours_modeles`.`Pays`, `edgecreator`.`tranches_en_cours_modeles`.`Magazine`, `edgecreator`.`tranches_en_cours_valeurs`.`Ordre`, `edgecreator`.`tranches_en_cours_valeurs`.`Nom_fonction`, `edgecreator`.`tranches_en_cours_valeurs`.`Option_nom`, `edgecreator`.`tranches_en_cours_valeurs`.`Option_valeur`
;