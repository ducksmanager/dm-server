...
docker exec ${container_name} rm -rf ${webdir}/{app,test,index.php,composer.json,deployment_commit_id.txt} \
&& docker exec ${container_name} /bin/bash -c "cp -rp ${webdir}_old/{app,test} ${webdir} && cp ${webdir}_old/{index.php,composer.json,deployment_commit_id.txt} ${webdir}" \
\
