...
for f in bin config scripts src templates translations .env .env.local .htaccess composer.json composer.lock docker-compose.yml deployment_commit_id.txt favicon.ico
do
  rm -rf "${webdir}/$f"
done
