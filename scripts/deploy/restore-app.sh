...
do
  rm -rf ${webdir}/{app,assets,test,index.php,composer.json,docker-compose.yml,deployment_commit_id.txt}
done

for f in app assets scripts test index.php composer.json
do
  cp -rp ${webdir}_old/{app,assets,test,index.php,composer.json,docker-compose.yml,deployment_commit_id.txt} "${webdir}"
done
